...
bool Queue_test_push_back(char* out_error_msg, int error_msg_length) {
    ...
    bool error = false;
    if (p1.tasks.first != &t1)                      error = true; else 
    if (strcmp(p1.tasks.first->name, t1.name) != 0) error = true;
    if (error) 
    {
        error = false;
        Return_Error("Expected the first pointer to be '%p', but got '%p'.", &t1, p1.tasks.first);
    }

    if (p1.tasks.last != &t4)                      error = true; else 
    if (strcmp(p1.tasks.last->name, t4.name) != 0) error = true;
    if (error) 
    {
        error = false;
        Return_Error("Expected the last pointer to be '%p', but got '%p'.", &t4, p1.tasks.last); 
    }

    bool error_link = false;
    if (p1.tasks.first != &t1)                   error_link = true; else 
    if (p1.tasks.first->next != &t2)             error_link = true; else 
    if (p1.tasks.first->next->next != &t3)       error_link = true; else 
...
bool Queue_test_pop_front(char* out_error_msg, int error_msg_length) {
    ...
    if (p1.tasks.first != p1.tasks.last) 
        Return_Error("Expected last to be '%p', but got '%p'.", p1.tasks.first, p1.tasks.last);
    ...
    bool error = false;
    if (strcmp(p1.tasks.first->name, t3.name) != 0) error = true; else
    if (p1.tasks.first->priority != t3.priority)    error = true; else
    if (p1.tasks.first != &t3)                      error = true;
    if (error)
    {
        error = false;
        Return_Error("Expected task '%s', but got '%s'.", t3.name, p1.tasks.first->name);
    }

    if (strcmp(p1.tasks.last->name, t4.name) != 0) error = true; else
    if (p1.tasks.last->priority != t4.priority)    error = true; else
    if (p1.tasks.last != &t4)                      error = true; 
    if (error)
    {
        error = false;
        Return_Error("Expected task '%s', but got '%s'.", t4.name, p1.tasks.last->name);
    }
